...
import asyncio, concurrent.futures, discord, itertools, json, os, ssl, time, urllib.request
...
try:
    TOKEN = open('token.txt', 'r', encoding='utf-8').read()  # Reading TOKEN from token.txt
except Exception:
    TOKEN = '' # TOKEN
...
class Artifacter(object):
    def __init__(self, uid):
        ...
        self.artifact_reality = ArtifactRarelity
        self.ArtifactFont = ArtifactFont
        self.uid = uid
        concurrent.futures.ThreadPoolExecutor().submit(self.Initialization)
    ...
    def Initialization(self):
        ...
        self.namecards = concurrent.futures.ThreadPoolExecutor().submit(self.namecard_json).result()  # キャラクターのネームカードデータをEnkaNetwork公式Githubから取得
        try:
            ...
            self.player_data = {}
        locked.append({'UID': self.uid, 'player_data': self.player_data})
    ...
    def namecard_json(self):
        return json.loads(urllib.request.urlopen(urllib.request.Request('https://raw.githubusercontent.com/EnkaNetwork/API-docs/master/store/namecards.json', headers={'User-Agent': UsrAgn})).read().decode(errors='ignore'))
    ...
    def full_playerinfo(self, avateriD, playerdata: dict = None):  # プレイヤーデータの完全版を取得
        try:
            avatar_data = []  # 事前に宣言
            if 'avatarInfoList' in playerdata:  # 取得したデータから「avaterInfoList」がある場合以下を実行
                avatar_data = list(filter(lambda item: item.get('avatarId') == int(avateriD), playerdata['avatarInfoList']))[0]
            if avatar_data == []:
    ...
    def create_buildcard(self, avaterinfo, score_state):
        ...
        with self.ArtifactFont() as Font:  # フォントファイルを一時フォルダへ展開後、フォントファイルを読み込み
            ...
            return BaseImage, self.uid

    def main(self, score_state, selected_chara='', lock: list = None):
        if lock == None:
            lock = locked[0:]
        if not len(lock) == 0:
            for data in lock:
                try:
                    ...
                    pass
                if not selected_chara == '':
                    yield self.create_buildcard(self.full_playerinfo(selected_chara, playerdata=data.get('player_data')), score_state)
                else:
                    yield self.create_buildcard(self.full_playerinfo(SelectedCharacter, playerdata=data.get('player_data')), score_state)
                break
            self.main(score_state, selected_chara, lock)
        else:
            if SelectedCharacter == '':
                yield self.create_buildcard(self.full_playerinfo(selected_chara), score_state)
            else:
                yield self.create_buildcard(self.full_playerinfo(SelectedCharacter), score_state)

class BuildModal(discord.ui.Modal):
    def __init__(self, member_id: int, *args, **kwargs) -> None:
        self.embed = None
        try:
            with open('user_uid.json', 'r', encoding='utf-8') as _u: # オートコンプリート機能
                d = json.load(_u)
            uid = d.get(str(member_id))
        except:
            uid = ''
        super().__init__(discord.ui.InputText(label="UIDを入力してください。", min_length=9, max_length=9, style=discord.InputTextStyle.short, value=uid, ), *args, **kwargs, )
    
    async def callback(self, ctx: discord.Interaction):
        global ArtifacterFunction
        uid = self.children[0].value
        ArtifacterFunction = Artifacter(uid)
        user_id.clear()
        locked.clear()
        try:
            int(self.children[0].value)
        except BaseException:
            return await ctx.response.send_message("UIDは半角英数字でかつ、数字で入力しなければいけません。", ephemeral=True)
        try:
            if ArtifacterFunction.player_data == '':
                await ctx.edit_original_response(content="キャラ詳細情報が非表示です。\nもう一度表示になっているか確認してまたお試しください。")
        except:
            pass
        try:
            with open('user_uid.json', 'r', encoding='utf-8') as uddr:
                data = json.load(uddr)
        except:
            try:
                with open('user_uid.json', 'w', encoding='utf-8') as ud:
                    ud.write('')
                    with open('user_uid.json', 'r', encoding='utf-8') as udr:
                        data = json.load(udr)
            except:
                data = {}
        data[str(ctx.user.id)] = uid
        with open('user_uid.json', 'w', encoding='utf-8') as j_w:
            json.dump(data, j_w, ensure_ascii=False, indent=4)
        concurrent.futures.ThreadPoolExecutor().submit(self.embed_profile)
        p = [0]
        for _p in p: # pycord独自のエラーに対処するために成功するまでループ
            try:
                try:
                    await ctx.response.send_message("プレイヤー情報を取得中...", ephemeral=True)
                except:
                    pass
                if not self.embed == None:
                    try:
                        await ctx.edit_original_response(content=None, embed=self.embed, view=playerview(uid, ctx))
                    except:
                        pass
                    break
            except:
                try:
                    ...
                    pass
            time.sleep(0.1) # 0.1秒間隔で実行
            p.append(_p + 1)

    def embed_profile(self):
        e = [0]
        for _e in e: # pycord独自のエラーに対処する為に成功するまでループ
            try:
                embed = discord.Embed(description=ArtifacterFunction.playerinfo()['signature'], color=discord.Color.orange())
                embed.set_author(name=ArtifacterFunction.playerinfo()['nickname'])
                embed.add_field(name='螺旋', value='{}層 {}間'.format(ArtifacterFunction.playerinfo()['towerFloorIndex'], ArtifacterFunction.playerinfo()['towerLevelIndex']))
                embed.add_field(name='アチーブメント', value=ArtifacterFunction.playerinfo()['finishAchievementNum'])
                embed.set_footer(text='冒険者ランク: {} / 世界ランク: {}'.format(ArtifacterFunction.playerinfo()['level'], ArtifacterFunction.playerinfo()['worldLevel']))
                try:
                    embed.set_thumbnail(url='https://enka.network/ui/{}.png'.format(ArtifacterFunction.characters_json['{}'.format(ArtifacterFunction.playerinfo()['profilePicture']['avatarId'])]['Costumes']['{}'.format(ArtifacterFunction.playerinfo()['profilePicture']['costumeId'])]['sideIconName'].replace('UI_AvatarIcon_Side_', 'UI_AvatarIcon_')))
                except:
                    embed.set_thumbnail(url='https://enka.network/ui/{}.png'.format(ArtifacterFunction.characters_json['{}'.format(ArtifacterFunction.playerinfo()['profilePicture']['avatarId'])]['SideIconName'].replace('UI_AvatarIcon_Side_', 'UI_AvatarIcon_')))
                embed.set_image(url='https://enka.network/ui/{}.png'.format(ArtifacterFunction.namecards['{}'.format(ArtifacterFunction.playerinfo()['nameCardId'])]['icon']))
                self.embed = embed
                break # 成功するとループから抜ける
            except:
                time.sleep(2) # 失敗すると２秒待機後に最初へ戻る
            e.append(_e+1)
...
class commandCog(commands.Cog):
    ...
    async def build(self, ctx: discord.ApplicationContext):
        await ctx.response.send_modal(BuildModal(member_id=ctx.author.id, title='UIDを入力'))


class charaSelect(discord.ui.Select):
    def __init__(self, uid, inters: discord.Interaction):
        self.uid = '{}'.format(uid)
        self.inters = inters
        options = [discord.SelectOption(label=ch['name'], description='Lv{}'.format(ch['level']), value='{}'.format(ch['avatarId'])) for ch in ArtifacterFunction.playerinfo()['showAvatarInfoList']]
        super().__init__(placeholder='キャラクターを選択', min_values=1, max_values=1, options=options)
    
    async def callback(self, interaction: discord.Interaction):
        global SelectedCharacter
        SelectedCharacter = self.values[0]
        await interaction.response.defer()
        if interaction.user != self.inters.user:
            return
        try:
            await self.inters.edit_original_response(content='読み込み中...', embed=None, view=None, attachments=[])
        except:
            await self.inters.edit_original_response(content='読み込み中...', embed=None, view=None)
        try:
            embed = concurrent.futures.ThreadPoolExecutor().submit(self.status__embed).result()
        except:
            embed = None
        try:
            await self.inters.edit_original_response(content=None, embed=embed, view=buildview(self.uid, self.inters, selected_ch=self.values[0]))
        except:
            try:
                await self.inters.edit_original_response(content='不明なエラーが発生しました。再度実行してください。', embed=None, view=None)
            except:
                await self.inters.delete_original_response()
    
    def status__embed(self):
        try:
            for cc in ArtifacterFunction.playerinfo()['showAvatarInfoList']:
                if '{}'.format(cc['avatarId']) == '{}'.format(self.values[0]):
                    if len('{}'.format(ArtifacterFunction.full_playerinfo(cc['avatarId'])['skillDepotId'])) == 3:
                        character_id = '{}-{}'.format(cc['avatarId'], ArtifacterFunction.full_playerinfo(cc['avatarId'])['skillDepotId'])
                    else:
                        character_id = '{}'.format(cc['avatarId'])
                    c_id = character_id
            embed = discord.Embed()
            cicon_url = 'https://enka.network/ui/{}.png'.format(ArtifacterFunction.characters_json['{}'.format(ArtifacterFunction.playerinfo()['profilePicture']['avatarId'])]['SideIconName'].replace('UI_AvatarIcon_Side_', 'UI_AvatarIcon_'))
            cname = ArtifacterFunction.locale_jp['{}'.format(ArtifacterFunction.characters_json[self.values[0]]['NameTextMapHash'])]
            clevel = '{}'.format(ArtifacterFunction.full_playerinfo(self.values[0])['propMap']['4001']['val'])
            tlent = 0
            try:
                tlent = len(ArtifacterFunction.full_playerinfo(self.values[0])['talentIdList'])
            except:
                pass
            ctext = '{} - Lv.{}\n{}'.format(cname, clevel, '命ノ星座: {}凸'.format(tlent))
            title_text = '{} - Lv.{}'.format(ArtifacterFunction.playerinfo()['nickname'], ArtifacterFunction.playerinfo()['level'])
            embed.set_author(icon_url=cicon_url, name=title_text)
            HPText = '**HP上限** : {}\n'.format(round(ArtifacterFunction.full_playerinfo(self.values[0])['fightPropMap']['2000']))
            AttckText = '**攻撃力** : {}\n'.format(round(ArtifacterFunction.full_playerinfo(self.values[0])['fightPropMap']['2001']))
            DefenceText = '**防御力** : {}\n'.format(round(ArtifacterFunction.full_playerinfo(self.values[0])['fightPropMap']['2002']))
            Elemental_M_Text = '**元素熟知** : {}\n'.format(round(ArtifacterFunction.full_playerinfo(self.values[0])['fightPropMap']['28']))
            Recharge_EfiText = '**元素チャージ効率** : {}\n'.format(round(ArtifacterFunction.full_playerinfo(self.values[0])['fightPropMap']['23'] * 100))
            CriticalRate = '**会心率** : {}％\n'.format(round(ArtifacterFunction.full_playerinfo(self.values[0])['fightPropMap']['20'] * 100))
            CriticalDamageRate = '**会心ダメージ** : {}％'.format(round(ArtifacterFunction.full_playerinfo(self.values[0])['fightPropMap']['22'] * 100))
            e_prop_id = {'Fire': '40', 'Electric': '41', 'Water': '42', 'Grass': '43', 'Wind': '44', 'Rock': '45', 'Ice': '46'}
            e_propjp = {'Fire': '炎元素', 'Electric': '雷元素', 'Water': '水元素', 'Grass': '草元素', 'Wind': '風元素', 'Rock': '岩元素', 'Ice': '氷元素'}
            e_prop_color = {'Fire': 0xFF3902, 'Electric': 0x9F72FF, 'Water': 0x3A78FF, 'Grass': 0x8BFF59, 'Wind': 0x59FF7D, 'Rock': 0xFFD071, 'Ice': 0x50FFFF}
            e_lm = ArtifacterFunction.characters_json['{}'.format(c_id)]['Element']
            ElementDameage = '**{}ダメージ** : {}\n'.format(e_propjp[e_lm], round(ArtifacterFunction.full_playerinfo(self.values[0])['fightPropMap']['{}'.format(e_prop_id[e_lm])] * 1000) / 10)
            Chter_Text = '{}{}{}{}{}{}\n{}{}'.format(HPText, AttckText, DefenceText, Elemental_M_Text, CriticalRate, CriticalDamageRate, Recharge_EfiText, ElementDameage)
            embed.add_field(name=ctext, value=Chter_Text, inline=False)
            embed.add_field(name='', value='\n')
            weapon_numbar = 0
            for i, w in enumerate(ArtifacterFunction.full_playerinfo(self.values[0])['equipList']):
                if 'weapon' in w:
                    weapon_numbar = i
            weapon_info = list(filter(lambda item: item.get('weapon'), ArtifacterFunction.full_playerinfo(self.values[0])['equipList']))[0]
            weapon = ArtifacterFunction.full_playerinfo(self.values[0])['equipList'][weapon_numbar]
            weapon_name = ArtifacterFunction.locale_jp['{}'.format(weapon['flat']['nameTextMapHash'])]
            weapon_level = weapon['weapon']['level']
            try:
                weapon_reary = list(weapon_info['weapon']['affixMap'].values())[0] + 1
            except:
                weapon_reary = 1
            weapon_value_t = ''
            for wp in weapon['flat']['weaponStats']:
                weapon_state_name = ArtifacterFunction.locale_jp['{}'.format(wp['appendPropId'])]
                weapon_state = wp['statValue']
                weapon_value_t += '**{}** : {}\n'.format(weapon_state_name, weapon_state)
            embed.add_field(name='{} - Lv.{}\n精錬ランク: {}'.format(weapon_name, weapon_level, weapon_reary), value=weapon_value_t, inline=False)
            embed.color = e_prop_color[e_lm]
            return embed
        except:
            embed = discord.Embed()
            return embed
...
class playerview(discord.ui.View):
    def __init__(self, uid, inters: discord.Interaction):
        ...
        self.inters = inters
        self.add_item(charaSelect(uid, inters))

    async def on_timeout(self):
        try:
            await self.inters.edit_original_response(view=None)
        except:
            pass
        return

class buildview(discord.ui.View):
    def __init__(self, uid, inters: discord.Interaction, selected_ch=''):
        super().__init__(timeout=120)
        self.inters = inters
        self.add_item(charaSelect(uid, inters))
        self.add_item(generation(uid, inters, selected_ch=selected_ch))
...
class generation(discord.ui.Select):
    def __init__(self, uid,  inters: discord.Interaction, selected_ch=''):
        self.inters = inters
        self.uid = uid
        self.selected_ch = selected_ch
        options = [discord.SelectOption(label='HP', description='HP基準でスコアを計算します', value='hp'), discord.SelectOption(label='攻撃', description='攻撃基準でスコアを計算します', value='attack'), discord.SelectOption(label='防御', description='防御基準でスコアを計算します', value='defence'), discord.SelectOption(label='チャージ', description='元素チャージ効率基準でスコアを計算します', value='charge'), discord.SelectOption(label='元素熟知', description='元素熟知基準でスコアを計算します', value='element')]
        super().__init__(placeholder='ビルドカード生成', min_values=1, max_values=1, options=options)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if interaction.user != self.inters.user:
            ...
            title = '{} ・ 世界ランク{} ・ 冒険者ランク{}'.format(ArtifacterFunction.playerinfo()['nickname'], ArtifacterFunction.playerinfo()['worldLevel'], ArtifacterFunction.playerinfo()['level'])
            embed = discord.Embed(title=title)
            user_id.append({'{}'.format(self.uid): {'userid': interaction.user, 'embed': embed}})
            score_state = ''
            if self.values[0] == 'hp':
                score_state = 'HP'
            elif self.values[0] == 'attack':
                score_state = '攻撃'
            elif self.values[0] == 'defence':
                score_state = '防御'
            elif self.values[0] == 'charge':
                score_state = 'チャージ'
            elif self.values[0] == 'element':
                score_state = '元素熟知'
            if not score_state == '':
                if not self.selected_ch == '':
                    await self.inters.edit_original_response(content='**処理中だよ！ちょっと待ってね！**', embed=None, view=None)
                    lpp = asyncio.get_event_loop()
                    await self.created_buildcard(score_state, lpp)

    async def created_buildcard(self, score_state, lpp):
        async def user_id_sync_loop(Buf, img): # 複数人同時入力に対処
            for i, user_sync in enumerate(user_id):
                try:
                    if user_sync['{}'.format(img[1])]['userid'] == self.inters.user:
                        embed = user_sync['{}'.format(img[1])]['embed']
                        File = discord.File(Buf, 'buildcard.png')
                        embed.set_image(url='attachment://buildcard.png')
                        await self.inters.edit_original_response(content=None, embed=embed, view=buildview(self.uid, self.inters), file=File)
                        time.sleep(1.55)
                    else:
                        embed = user_sync['{}'.format(self.uid)]['embed']
                        File = discord.File(Buf, 'buildcard.png')
                        embed.set_image(url='attachment://buildcard.png')
                        await self.inters.edit_original_response(content=None, embed=embed, view=buildview(self.uid, self.inters), file=File)
                    break
        ...
        for img in await lpp.run_in_executor(concurrent.futures.ThreadPoolExecutor(), ArtifacterFunction.main, score_state, self.selected_ch):
            Buf = BytesIO()
            img[0].save(Buf, 'png')
            Buf.seek(0)
            await user_id_sync_loop(Buf, img)
        user_id.clear()
        locked.clear()

@Bot.event
async def on_ready():
    await Bot.change_presence(activity=discord.Game('Bot is now loaded!'))

@Bot.slash_command(description="ビルドカード作成ボタンを設置できます。")
async def start(ctx):
    embed = discord.Embed(title='ビルドカード', description='**【使い方】**\n> "ビルドカードを生成" ボタンを押してください。\n> 使い終わったら操作を終了してください。\n\n**【スコア計算式】**\n> 本機能ではスコア平滑化の為、以下のように計算式を調整しています。\n> `スコア = 会心率×2 + 会心ダメージ + HP`\n> `スコア = 会心率×2 + 会心ダメージ + 攻撃力`\n> `スコア = 会心率×2 + 会心ダメージ + 防御力×0.8`\n> `スコア = 会心率×2 + 会心ダメージ + 元素熟知×0.25`\n> `スコア = 会心率×2 + 会心ダメージ + 元素チャージ×0.9`')
    embed.color = 0xDD0BCD
    button = discord.ui.Button(label='ビルドカードを生成', style=discord.ButtonStyle.green)
    view = discord.ui.View(timeout=None)
    view.add_item(button)
    async def callback(interation: discord.ApplicationContext):
        await interation.response.send_modal(BuildModal(member_id=interation.user.id, title='UIDの入力'))
...
def uptimecounter():
    ...
    concurrent.futures.ThreadPoolExecutor().submit(timecount)
